...
f.open(path , ios::in | ios::binary);
   ...
   if(!f.is_open())
   {
      cout << "FATAL ERROR , file coudn`t be read!" << endl;
      return;
   }
   ...
   {
      cout << "The specified path isn`t a bitmap image " << endl ;
      f.close();
      return;
...
void Image::Export(const char* path) const
{
   ...
   if(!f.is_open())
   {
      cout << "File coudn`t be open!" << endl ;
      return;
   }
   ...
   const int fileHeaderSize = 14;
   const int informationHeaderSize = 40 ;
   const int fileSize = fileHeaderSize * informationHeaderSize  + m_width * 3 + paddingAmount * m_width ; 
